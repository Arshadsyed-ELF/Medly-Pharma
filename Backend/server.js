...
app.use(cors({
    origin: ["https://medly-pharma-1.onrender.com"],
...
app.post('/items', upload.single('itemImage'), async (req, res) => {
    ...
    try {
        const item = new items({ itemtype, description, price, itemImage, userId, userName });
        await item.save();
...
app.post('/userorder', async (req, res) => {
    const { flatno, city, state, pincode, totalamount, seller,sellerId, BookingDate, description, Delivery, userId, userName: String,itemname,itemImage } = req.body;

    try {
        const bike = new myorders({ flatno, city, state, pincode, totalamount, seller,sellerId, BookingDate, description,userId,Delivery, userName: String,itemname,itemImage });
        await bike.save();
...
app.post('/items', upload.single('itemImage'), async (req, res) => {
    ...
    const itemImage = req.file.path; // The path to the uploaded image
    try {
        const item = new wishlist({ itemtype, description, price, itemImage, userId, userName });
        await item.save();
        res.status(201).json(item);
    } catch (err) {
        res.status(400).json({ error: 'Failed to create item' });
