...
const Vendors = () => {
  ...
  const toggleDetail = () => {
   ...
   useEffect(() => {
    axios.get(`http://localhost:8000/getvendors`)
      .then((response) => {
...
const deleteData = (taskId) => {
    axios.delete(`http://localhost:8000/vendordelete/${taskId}`);
    window.location.assign('/vendors');
    alert('User is deleted');
  };
  const deleteitem = (taskId) => {
    axios.delete(`http://localhost:8000/useritemdelete/${taskId}`);
  ...
  const fetchUserBikeData = (userId) => {
    ...
    axios.get(`http://localhost:8000/getitem/${userId}`)
                            ...
                            return (
                              ...
                              >
                                <div style={{ display: 'flex', justifyContent: 'space-around' }}>
                                  <div>
                                    <img src={`http://localhost:8000/${item?.itemImage}`} alt={`${item.itemtype} Image`} style={{ height: "80px",width:"120px" }} /> <br/>
                                  </div>
                                  <div>
                                    <p>Product Name:</p>
