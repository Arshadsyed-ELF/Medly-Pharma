...
function OrderItem() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!item || !item.userName || !item.userId || !item.description || !item.price || !item.itemtype || !item.itemImage) {
        throw new Error('Item data is missing required properties');
      }

      const { userName, description, price,itemtype,itemImage,userId } = item;
             ...
             {item && (
                <div>
                  ...
                  <div style={{ height: "100px",width:"120px" }} >
                        ...
                        <p className='text-end'>{item.itemtype}</p>
                    </div>
                  </div>
                    ...
                    <div style={{ display:'flex',justifyContent:"space-between" }}>
                        ...
                        <div style={{ display:'flex',justifyContent:"space-between" }}>
                            <p style={{ fontSize: "17px" }}>Conveience Fee:</p>
                            <p>29</p>
                        </div>
                        <div style={{ display:'flex',justifyContent:"space-between" }}>
